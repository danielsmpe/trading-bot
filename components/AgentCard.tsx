import { ArrowDownIcon, ArrowUpIcon, SunIcon as SolanaIcon } from "lucide-react"
...
export function AgentCard({ name, pnlPercentage, pnlAmount, investedAmount, investedValue }: AgentCardProps) {
    ...
    <div className={`bg-gradient-${gradientColor} p-[1px] rounded-lg`}>
      <div className="bg-black p-6 rounded-lg h-full">
        ...
        <div className="flex items-center mb-4">
          <span className={`text-2xl font-bold text-gradient-${gradientColor}`}>{pnlPercentage.toFixed(2)}%</span>
          <ArrowIcon className={`ml-2 text-${gradientColor}-400`} size={20} />
        </div>
        <div className="flex items-center mb-4">
          <SolanaIcon className="mr-2 text-purple-400" size={20} />
          <span className="text-lg font-semibold">{pnlAmount.toFixed(2)} SOL</span>
        </div>
        <div className="mb-4">
